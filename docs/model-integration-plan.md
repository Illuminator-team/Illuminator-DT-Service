...
- PV uses an explicit 2024 reference-weather calendar and 2035 scenario year, while the current Consumption slice uses a 2023 calendar. Both remain independently usable; they MUST NOT be added into one net profile until an explicit calendar-alignment policy is selected and tested.
