...
- The first PV profile slice remains PV-only. The policy backend requests a feature-linked PT15M production profile, asks Grid for the CBS buurt hierarchy assignment, and asks Congestion for transient LV/MV and MV/HV aggregates. It does not alter `/simulate` or combine PV with Consumption yet.
- PV uses an explicit 2024 reference-weather calendar and 2035 scenario year, while the current Consumption slice uses a 2023 calendar. The temporary MVP alignment policy is now explicit: remove 29 February from PV and project the remaining ordinal PT15M intervals onto the fixed 2023 Consumption axis. Record this as a modeling limitation, set the combined baseline `datacompleetheid` to 1/3, and replace it with a shared-calendar policy before professional release.
- Grid owns the area-to-network mapping. For CBS buurten, it uses active BAG VBO counts to distribute the buurt over PC6 areas, then applies the Grid-owned PC6 transformer shares. Missing authoritative PC6 geometry, incomplete weights, or ambiguous upstream topology remains unavailable; downstream services MUST NOT drop or renormalize those shares.
- The first positive PV hierarchy fixture is `BU03610709` (`Landelijk gebied Noord`): its BAG weights are complete and all targets resolve provisionally to OS WARMENHUIZEN. The compact deterministic fixture preserves three previously accepted LV/MV paths; the official 2024 CBS polygon intersects two paths in the real-snapshot inventory. Both cases conserve a total share of 1.0 and remain separate evidence lanes. `BU03610302` (`Overdie-Oost`) remains an explicit fail-closed regression because one nonzero LV/MV share is tied between OS OUDORP and OS ALKMAAR. `BU03610308` remains unavailable where authoritative geometry for PC6 `1812RW` is absent.
...
- PV production-profile acceptance uses a daylight window and requires at least one positive source value before Congestion normalization. The resulting transformer points use canonical signed kW: demand/import is positive and production/export is negative. Both aggregation stages must conserve the signed interval totals.
...
- use crawler/ingestion outputs for sources that have been promoted because they are shared or operationally mature; unpromoted prototype sources may remain behind model-owned input adapters;
...
Models own their external source connections while those sources are still being prototyped. A source moves to the existing RDP crawler/ingestion services when its contract is stable and shared reuse or operational concerns justify central ownership.

Preferred flow:

```text
promoted external source -> RDP crawler/ingestion -> Redis/Timescale/PostGIS -> model service -> layer publisher/GeoServer/API -> frontend
```

For unpromoted sources, direct model access is allowed in both standalone development and early integration, but every connection stays behind an input adapter so it can later switch to crawler-managed data without changing model calculations.
...
15. Promote an external source into the existing crawler/ingestion services only after its contract stabilizes and it is shared by multiple models or needs centralized scheduling, caching, retries, provenance, or credentials; preserve standalone adapters and fixtures.
16. Start phase 4 hardening only after the working map/model/scenario flow is stable in `dev`.

## Decision Status

The initial thirteen-question architecture review is complete. New implementation questions should be recorded when concrete model repositories, data contracts, and integration stories expose them.
